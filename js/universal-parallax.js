...
// position parallax
var positionParallax = function positionParallax(
  container,
  speed,
  parallax,
  elem
...
var animateParallax = function animateParallax(parallax, speed) {
  for (var i = 0; parallax.length > i; i++) {
    var container =
      parallax[i].parentElement.parentElement.getBoundingClientRect();

    // only animate if on screen
    if (
      container.top + container.height >= 0 &&
      container.top <= windowHeight
    ) {
...
var universalParallax = function universalParallax() {
  ...
  this.init = function (param) {
    ...
    for (var i = 0; parallax.length > i; i++) {
      ...
      if (
        window
          .getComputedStyle(parallaxContainer.parentElement, null)
      ...
      if (typeof imgData !== "undefined") {
        ...
        if (
          parallax[i].classList.length === 1 &&
          parallax[i].classList[0] === "parallax--bg"
        ) {
